...
export const SocialLogin = ({ setError }) => {
  ...
  const createAccountWithGoogle = async (apiUrl) => {
    ...
    try {
      ...
      setIsCreatingAccount(false); // Reset flag
    }
  };
  ...
  const fetchUserProfile = async () => {
    ...
    const endpoint =
      role === "buyer"
        ? `http://api.mymakan.ae/user/${userId}`
        : `http://api.mymakan.ae/agent/${userId}`;
    try {
  ...
  useEffect(() => {
    if (status === "authenticated" && session?.user) {
      const apiUrl =
        role === "buyer"
          ? `http://api.mymakan.ae/auth/user/signUp-with-Google`
          : `http://api.mymakan.ae/auth/agent/signUp-with-gmail`;
  ...
  return (
    <div className="flex gap-3 justify-center">
      <button
        ...
        className="py-2.5 pl-4 text-sm font-semibold rounded text-blue-500 bg-blue-100 hover:bg-blue-200 focus:outline-none"
      >
        <svg
